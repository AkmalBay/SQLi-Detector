...
<div class="max-w-2xl mx-auto">
    ...
    <div class="bg-white dark:bg-dark-card rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 p-8">
        <h2 class="text-xl font-bold text-gray-800 dark:text-white mb-6">Edit Informasi Karyawan</h2>
        ...
        <form action="{{ route('employees.update', $employee) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $employee->name) }}" class="w-full px-4 py-2 rounded-xl border border-gray-300 focus:border-brand-blue focus:ring-2 focus:ring-brand-blue/20 outline-none transition-all dark:bg-dark-bg dark:border-gray-700 dark:text-white" required>
                @error('name')
            ...
            <div>
                ...
                <input type="email" name="email" id="email" value="{{ old('email', $employee->email) }}" class="w-full px-4 py-2 rounded-xl border border-gray-300 focus:border-brand-blue focus:ring-2 focus:ring-brand-blue/20 outline-none transition-all dark:bg-dark-bg dark:border-gray-700 dark:text-white" required>
                @error('email')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="border-t border-gray-200 dark:border-gray-700 pt-6">
                <h3 class="text-md font-bold text-gray-800 dark:text-white mb-4">Ubah Password <span class="text-sm font-normal text-gray-400">(Opsional - Kosongkan jika tidak ingin mengubah)</span></h3>
